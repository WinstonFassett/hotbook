...
Read of `inspo/bireactive/src/` to ground vizform's diagram-kit plans in
...
## What's actually new in vizform's spike code
...
1. Should `vizform-diagram` (or whatever it's named) live in the same
...
2. Where do d3 adapters live — `bireactive-d3` as a peer to bireactive, or
   `vizform-diagram/backends/d3` as an internal? Lean toward peer
   package, since "d3 layouts under bireactive cells" is a generally
   useful pattern beyond vizform.
...
4. The `cross-view.ts` morph between hierarchical viz types is one of
   the most evocative things in the spike. It's vizform-original but
