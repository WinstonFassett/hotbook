# fiddleviz Viewer Architecture — diagram / viewer / container
...
3. **Container** — the pixel box itself (a fiddleviz tile, or an auto-height
...
- **Viewer as a bireactive primitive?** If the Viewer is a bireactive
  `Diagram`-level construct, fiddleviz tiles could host bireactive viewers
  **natively** rather than through React wrappers — which lines up with the
  standing "replace React with Svelte in fiddleviz" direction
...
4. **Evaluate:** bireactive-native viewers in fiddleviz vs. React wrappers.
