...
- **Drill** — same model (an `updated`, rendered as an autonomous `transition`), geometry differs:
  ...
  - **Viewport tween** animates the angular domain `[va0, va1]` and radial domain `[vr0, vr1]` — the radial analog of the icicle's affine layout transform. All arcs (present and exiting) interpolate through the viewport remap so an interrupted drill restarts from the current visual position (rule 13). Interruptible/disposable, same as icicle.
