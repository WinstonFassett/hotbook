...
- **Drill — affine viewport zoom (diverges from icicle/treemap's relayout).** The viewport is a layout-space bounding box `[vx0, vy0, vx1, vy1]`; on drill it tweens to the union bounding box of the focus node's descendants. A **uniform** `min(Wc/spanW, Hc/spanH)` scale maps layout-space to canvas — required because circles must stay circular. Otherwise the drill model follows the family contract: an `updated`, rendered as an autonomous `transition`, interruptible/disposable (rule 13), enter/exit lifecycle, no continuous drill, no preview.
