...
<!--
  Icicle = d3.partition in cartesian coords.
...
<script lang="ts">
  ...
  import { Chart, Svg, Partition, Group, Rect, Bounds } from "layerchart";
              ...
              >
                ...
                {#if width > 28 && height > 12}
                  <text
                    ...
                    pointer-events="none"
                  >
                    {n.data.value.label}{#if isLeaf}<tspan x={width / 2} dy="1.2em" font-size="9">{n.data.value.total.value.toFixed(0)}</tspan>{/if}
                  </text>
                {/if}
              </Group>
            {/if}
