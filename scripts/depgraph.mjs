...
// Pull vite source aliases from sliceboard (dotted edges).
const aliasEdges = []
const viteCfg = join(root, 'apps/sliceboard/vite.config.ts')
if (existsSync(viteCfg)) {
  ...
  for (const m of txt.matchAll(/'(@[\w-]+)':\s*path\.resolve\([^,]+,\s*'([^']+)'\)/g)) {
    ...
    if (target) aliasEdges.push({ from: 'sliceboard', alias: m[1], dir: target })
  }
}
...
for (const e of aliasEdges) {
  ...
  if (dirNode) lines.push(`  ${id('sliceboard')} -. "${e.alias}" .-> ${id(dirNode.name)}`)
}
lines.push('```')
