 ...
 *
 * Takes a flat PNode array with parentId references and builds a hierarchical
 * TreeNode structure. Roots become direct children of a synthetic __root__ node.
 *
 * @param rows - Flat array of PNode objects with parentId references
 * @param measureKey - Primary measure key (used for D3 value)
 * @returns Tree structure with __root__ at the top
 */
export function buildTree(rows: PNode[], measureKey: string): TreeNode {
 ...
 *
 * @param rows - PNode array (typically flat, but non-root nodes are preserved)
 ...
 */
export function applyView(rows: PNode[], dimKey: string): PNode[] {
 ...
 *
 * @param rows - PNode array to search through
 * @param drillNodeId - ID of target node (or null to return empty array)
 * @returns Array of PNodes from root to target; empty if not found
 */
 ...
 *
 * @param rows - PNode array
 * @returns All leaf PNodes
 */
export function leavesOf(rows: PNode[]): PNode[] {
