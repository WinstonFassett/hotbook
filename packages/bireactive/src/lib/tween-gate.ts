import { type Cell, tween, easeOut, type Easing } from "bireactive";
 ...
 *
 * @param opts.cell - The reactive cell to update
 ...
 */
export function applyWithTweenGate<T extends number>(opts: {
  cell: Cell<T>;
  target: T;
  structural: boolean;
  host: HTMLElement;
  anim: { start(...generators: Generator<void, void, number>[]): () => void };
  duration?: number;
  easing?: Easing;
}): (() => void) | null {
  const {
 ...
 */
export function applyMultiWithTweenGate<T extends number>(opts: {
  updates: Array<{ cell: Cell<T>; target: T }>;
  structural: boolean;
  host: HTMLElement;
  anim: { start(...generators: Generator<void, void, number>[]): () => void };
  duration?: number;
