...
## Part 4 — flat-chart settle coverage (WIN-47)

Settle transitions extended from bar to line / area / pie / radar / concentric-arc using the same helper module + gesture-suppression contract.

| Chart | Animated element | Property | Chrome | Safari | Firefox |
| --- | --- | --- | :-: | :-: | :-: |
| bar | `<rect>` per bar | `y` / `height` / `width` / `fill` | ✓ | ✓ | ✓ |
| line | `<path>` (spline) | `d` | ✓ | ✓ | step |
| area | `<path>` × 2 (area band + spline) | `d` | ✓ | ✓ | step |
| pie | `<path>` per slice (annularSector) | `d` | ✓ | ✓ | step |
| radar | `<path>` × 2 (fill + stroke polygon) | `d` | ✓ | ✓ | step |
| concentric-arc | `<path>` per ring value arc | `d` | ✓ | ✓ | step |

**The path-d caveat.** Firefox does not interpolate the SVG `d` attribute as a CSS property — it steps from old to new at the end of the transition duration regardless of the declared timing. Chrome and Safari interpolate per the SVG WG spec. We accept the step on Firefox as graceful degradation: the *value* still updates synchronously (no flush gap, no jank), only the *visual settle* is skipped. The settle is a polish layer, not a correctness layer, and the gesture-suppression contract is unaffected. If smoothness on Firefox matters for a specific chart later, the targeted fix is a tween cell that interpolates the input numbers and re-derives `d` per frame — paying the per-property tween cost only where it's worth it.

**Radar uses path-d, not `<polyline points>`.** The radar polygon is rendered with `<path>` + `M`/`L` commands, so it benefits from Chrome/Safari's path-d interpolation just like the other shape charts. (Plain `<polyline points>` is not CSS-animatable in any engine — that's the reason we chose path-d, not a limitation we ship.)

**Cartesian gesture helper carries the contract.** `attachCartesianGestures` (shared by line / area / scatter) now toggles `GESTURE_ACTIVE_CLASS` from inside its own wheel/drag configs, so any future cartesian chart picks up gesture suppression for free without touching the call site.

**Concentric-arc cleanup.** Pre-existing inline `transition: "d 0.1s"` (value arc) and `"opacity 0.12s"` (handle) replaced with `settleTransition("d")` / `hoverTransition("opacity")` so timing lives in one place.
