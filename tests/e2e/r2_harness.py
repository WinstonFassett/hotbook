...
Two assertions every chart in the sweep must pass:
  ...
  check_value_immediate(chart_tag, ...)
      ...
      hit (window.__fiddleviz.setCell → commit(updateRow(...))) must reach final
...
Value-edit driver: `window.__fiddleviz.setCell(dsId, rowId, measureKey, v)` is
registered by apps/fiddleviz/src/main.ts (DEV builds only) and takes the exact
...
Env: BASE_URL (default http://fiddleviz.localhost:1355). Point at a Netlify
...
class R2Harness:
    ...
    def _require_hook(self):
        """Fail loudly if the DEV-only __fiddleviz hook is missing (production build
        or old commit). Every value_immediate check depends on it."""
        ok = self.page.evaluate(
            "() => typeof window.__fiddleviz === 'object' && typeof window.__fiddleviz.setCell === 'function'"
        )
        if not ok:
            raise RuntimeError(
                "window.__fiddleviz.setCell is not available — is this a DEV build of fiddleviz? "
                "The hook is registered in apps/fiddleviz/src/main.ts under `if (import.meta.env.DEV)`."
            )
    ...
    def _drive_edit(self, tag: str):
        ...
        Row picking order (first that works):
          ...
          3. window.__fiddleviz.rowIds(dsId) — first row of the active dataset;
             works for any chart that renders every dataset row.
        Measure key picking order:
          1. el.measureKey property (all fiddleviz charts expose this).
          2. window.__fiddleviz.measureKeys(dsId)[0] — first numeric measure.
        """
        info = self.page.evaluate(
            """(tag) => {
              const el = document.querySelector(tag);
              if (!el) return {ok:false, reason:'no element'};
              const dsId = window.__fiddleviz.activeDatasetId();
              if (!dsId) return {ok:false, reason:'no active dataset'};
              const measureKey = el.measureKey || window.__fiddleviz.measureKeys(dsId)[0];
              if (!measureKey) return {ok:false, reason:'no measureKey'};
              const allIds = window.__fiddleviz.rowIds(dsId);
              ...
              for (const cand of candidates) {
                const v = window.__fiddleviz.getCell(dsId, cand, measureKey);
              ...
              window.__fiddleviz.setCell(dsId, rowId, measureKey, next);
